  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", padding: "32px", backgroundColor: "#f9fafb", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        ...
        <h1 style={{ margin: "8px 0 4px", color: "#1E3A5F", fontSize: "24px" }}>🔍 ค้นหาข้อมูล</h1>
        ...
        <div style={{ display: "flex", gap: "10px", maxWidth: "600px" }}>
          <input
            ...
            style={{
              ...
              border: "2px solid #1E3A5F", fontSize: "15px",
                ...
                {assetResults.map((a, i) => {
                  ...
                  return (
                    <tr key={a.id} style={{ backgroundColor: i % 2 === 0 ? "white" : "#f8fafc", borderBottom: "1px solid #f1f5f9" }}>
                      <td style={{ padding: "10px 12px", fontSize: "18px" }}>{TYPE_ICON[a.assetType] ?? "📦"}</td>
                      <td style={{ padding: "10px 12px", fontWeight: 700, color: "#2563EB", fontFamily: "monospace", fontSize: "12px" }}>{a.assetCode}</td>
                      ...
                      <td style={{ padding: "10px 12px", fontFamily: "monospace", fontSize: "11px", color: "#6b7280" }}>{a.partNumber ?? "—"}</td>
                      <td style={{ padding: "10px 12px", fontWeight: 600 }}>{a.customer.companyName}</td>
                      <td style={{ padding: "10px 12px", color: "#6b7280", fontSize: "12px" }}>{a.site?.siteName ?? "—"}</td>
                      <td style={{ padding: "10px 12px", fontSize: "12px" }}>
                        {a.warrantyEnd ? a.warrantyEnd.toLocaleDateString("th-TH") : "—"}
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{ color: wb.color, fontWeight: "bold", fontSize: "12px" }}>{wb.label}</span>
                      </td>
                      ...
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{
                          ...
                          padding: "2px 8px", borderRadius: "99px", fontSize: "11px", fontWeight: "bold"
                        }}>
                ...
                {contractItemResults.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id} style={{ backgroundColor: i % 2 === 0 ? "white" : "#f8fafc", borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "10px 12px", fontWeight: 700, color: "#7c3aed", fontFamily: "monospace", fontSize: "12px" }}>{item.contract.contractNo}</td>
                      <td style={{ padding: "10px 12px", fontFamily: "monospace", fontSize: "12px", color: "#6b7280" }}>{item.partNumber ?? "—"}</td>
                      ...
                      <td style={{ padding: "10px 12px", fontSize: "12px" }}>{item.sla ?? "—"}</td>
                      <td style={{ padding: "10px 12px", fontSize: "12px" }}>{item.startDate ? new Date(item.startDate).toLocaleDateString("th-TH") : "—"}</td>
                      <td style={{ padding: "10px 12px", fontSize: "12px" }}>
                        <span style={{ color: dayColor, fontWeight: "bold" }}>
                          {item.endDate ? new Date(item.endDate).toLocaleDateString("th-TH") : "—"}
                          {days !== null && <span style={{ fontSize: "11px", marginLeft: "4px" }}>({days < 0 ? `${Math.abs(days)}d ago` : `${days}d`})</span>}
                        </span>
                      </td>
                      <td style={{ padding: "10px 12px" }}>
                        <span style={{ fontSize: "11px", fontWeight: "bold", color: item.contract.status === "ACTIVE" ? "#10b981" : "#6b7280" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "12px" }}>
            {customerResults.map((c) => (
              <div key={c.id} style={{
                ...
                <div style={{ fontWeight: 700, color: "#1E3A5F", fontSize: "14px", marginBottom: "4px" }}>{c.companyName}</div>
                {c.shortName && <div style={{ color: "#6b7280", fontSize: "12px", marginBottom: "6px" }}>{c.shortName}</div>}
                {c.contactPerson && <div style={{ fontSize: "13px", marginBottom: "2px" }}>👤 {c.contactPerson}</div>}
                {c.contactPhone && <div style={{ fontSize: "13px", color: "#6b7280" }}>📞 {c.contactPhone}</div>}
      ...
      {query && total === 0 && (
        <div style={{
          ...
          <div style={{ fontSize: "16px", fontWeight: 600, marginBottom: "6px" }}>ไม่พบข้อมูล</div>
          <div style={{ fontSize: "14px" }}>ลองค้นหาด้วย Serial Number, ชื่อลูกค้า, หรือ Part Number</div>
        </div>
      )}
    </div>
  );
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: "24px" }}>
      <h2 style={{ color: "#1E3A5F", fontSize: "16px", marginBottom: "12px", fontWeight: 700 }}>{title}</h2>
